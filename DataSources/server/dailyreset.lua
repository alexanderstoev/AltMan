...
function AltMan.DataSources.dailyreset()
    ...
    local _drString =
        AltMan.translations["en"]["maininfo"]["dailyreset"]["daily"];
    local _secToDaily = C_DateAndTime.GetSecondsUntilDailyReset();

    if (_secToDaily < AltMan.constants.secondsinhour) then
        _drString = _drString ..
                        AltMan.translations["en"]["maininfo"]["dailyreset"]["lessthanhour"];
    else
        _drString = _drString ..
                        AltMan.translations["en"]["maininfo"]["dailyreset"]["in"] ..
                        math.floor(0.5 + _secToDaily /
                                       AltMan.constants.secondsinhour) .. -- hacky way for rounding
